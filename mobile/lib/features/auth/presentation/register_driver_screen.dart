    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: ListView(
          ...
          children: [
            Text('Профиль водителя', style: Theme.of(context).textTheme.headlineSmall),
            const SizedBox(height: 8),
            Text('Водитель сможет создавать поездки только после проверки администратора.', style: Theme.of(context).textTheme.bodySmall),
            ...
            const SizedBox(height: 12),
            TextField(controller: licenseNumber, decoration: const InputDecoration(labelText: 'Номер водительского удостоверения')),
            const SizedBox(height: 10),
            const _UploadPlaceholder(text: 'Фото профиля, selfie, паспорт, права'),
            const SizedBox(height: 20),
            Text('Автомобиль', style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: 12),
            TextField(controller: carBrand, decoration: const InputDecoration(labelText: 'Марка')),
            const SizedBox(height: 12),
            TextField(controller: carModel, decoration: const InputDecoration(labelText: 'Модель')),
            const SizedBox(height: 12),
            TextField(controller: carColor, decoration: const InputDecoration(labelText: 'Цвет')),
            const SizedBox(height: 12),
            TextField(controller: carYear, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Год')),
            const SizedBox(height: 12),
            TextField(controller: plateNumber, decoration: const InputDecoration(labelText: 'Госномер')),
            const SizedBox(height: 12),
            DropdownButtonFormField(
              value: seats,
              decoration: const InputDecoration(labelText: 'Количество мест'),
              items: [1, 2, 3, 4, 5, 6, 7, 8].map((item) => DropdownMenuItem(value: item, child: Text('$item'))).toList(),
              onChanged: (value) => setState(() => seats = value!),
            ),
            const SizedBox(height: 10),
            const _UploadPlaceholder(text: 'Техпаспорт, фото спереди, сзади и салона'),
            if (error != null) ...[
              const SizedBox(height: 12),
            ...
            if (success != null) ...[
              const SizedBox(height: 12),
              Text(success!, style: const TextStyle(color: Color(0xFF047857), fontWeight: FontWeight.w700)),
            ],
          ],
        ),
      ),
      bottomNavigationBar: SafeArea(
        ...
        child: ElevatedButton(
          ...
          child: loading ? const CircularProgressIndicator.adaptive() : const Text('Отправить на проверку'),
        ),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: const Color(0xFFD1FAE5).withOpacity(0.35),
